...
You can include or exclude bug detection for particular classes and methods using [filters](https://spotbugs.readthedocs.io/en/latest/filter.html) with the settings `findbugsIncludeFilters` and `findbugsExcludeFilters`.
...
To use SpotBugs plugins such as [fb-contrib](http://fb-contrib.sourceforge.net) or [find-sec-bugs](http://find-sec-bugs.github.io) use the `findbugsPluginList` setting:
...
SpotBugs comes with a number of default XSL files which you can use, these are found in [`findbugs/src/xsl`](https://github.com/JosephEarl/findbugs/tree/master/findbugs/src/xsl).
